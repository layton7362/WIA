...
Blochs Definition ist dabei etwas Allgemeiner. Laut Bloch \cite{bloch2014} handelt es sich um eine \ac{API}, wenn sie folgende Kriterien erfüllt:
\begin{enumerate}
...
Der Name der Schnittstelle lautet linkStrings, ihre Eingaben sind Zwei Strings, mit den Bezeichnern head und tail. Bei der Ausgabe der Methode handelt es sich um den Rückgabewert, welches in diesem Fall als Typ auch ein String ist.
...
\subsection{Keine Konsistente Methodenbezeichner}

Wenn sich ein Entwickler in einer neuen Bibliothek einarbeiten soll, dann gibt es mehrere Ansätze wie sich der Entwickler einarbeiten kann. Zunächst kann er dies tun, indem er die Dokumentation der Schnittstellen der Bibliothek ansieht\ref{MD}. Allerdings kann dies auch, oder zusätzlich, durch Ausprobieren der Schnittstellen geschehen. --TODO-- Verweis auf irgendwas
...
Der Name einer Methode sollten idealerweise ihren Zweck beschreiben. Einen Namen gut zu wählen kann viel Zeit kosten, allerdings erleichtert es dem Entwickler, welche eine API nutzt, die Suche nach einer Passenden Methode\cite[siehe Seite 45f]{martin2009}. Zudem Hilft es auch beim Verbessern der API, da so einige 
